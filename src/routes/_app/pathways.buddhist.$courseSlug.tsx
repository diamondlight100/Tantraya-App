...
import { mettaTonglenChapters, type BuddhistChapter } from "@/data/buddhist/metta-tonglen";
import { phowaChapters } from "@/data/buddhist/phowa";
...
import { TransitionCard } from "@/components/course/phowa-widgets";

export const Route = createFileRoute("/_app/pathways/buddhist/$courseSlug")({
  head: ({ params }) => ({
    meta: [{ title: `${params.courseSlug} · Buddhist · Tantraya` }],
  }),
  component: BuddhistCourse,
  notFoundComponent: () => (
...
const courses = {
  "metta-tonglen": {
    ...
    chapters: mettaTonglenChapters,
  },
  phowa: {
    title: "Beyond the Threshold: Phowa for Modern Times",
    subtitle:
      "Phowa, the transference of consciousness, adapted for practitioners of any background. Seventeen chapters move from the subtle body and ethical ground through every stage of the practice, closing with the complete sequence, a printable Conscious Transition card, and further reading.",
    youtubeId: undefined as string | undefined,
    chapters: phowaChapters,
  },
} as const;

function ChapterWidget({ kind }: { kind?: BuddhistChapter["widget"] }) {
  switch (kind) {
...
function ChapterCard({ chapter, courseSlug }: { chapter: BuddhistChapter; courseSlug: string }) {
